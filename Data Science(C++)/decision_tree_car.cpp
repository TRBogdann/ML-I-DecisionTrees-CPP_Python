...
class DecisionTreeNode
{
    ...
    {   
        ...
        int a = encoddings.at(row[node->column_index]);
        int b = encoddings.at(node->median);
    ...
    float test(const DataSet& set,const SplitSets& splits,int index,int med,float& entropyL,float& entropyR)
    {   
        ...
        for(auto it=splits.sets[index].begin();it!=splits.sets[index].end();it++)
        {
            ...
            for(auto it=splits.sets[index].begin();it!=splits.sets[index].end();it++)
            {
                encoddings.insert((std::pair<std::string,int>{*it,stoi(*it)}));   
            }
        }
            ...
            for(auto it=splits.sets[index].begin();it!=splits.sets[index].end();it++)
            {
                encoddings.insert((std::pair<std::string,int>{*it,stof(*it)}));   
            }
        }
    ...
    DecisionTreeNode(const DataSet& set,const SplitSets& splits,float entropy)
    {
        ...
        {
        SplitSets** newSplitSets = splits.split(savaedIndex,savedValue);
    ...
    DecisionTree(const DataSet& set) : dataSet(set)
{
    ...
    {
        if (dataSet.meta[i].dataType == atrib)
        {
            count++;
        }
    }
